...
generate.data.poly <- function(N, seed, sd.noise = 300) {
  ...
  q <- seq(from=-15, to=-0.001, length.out = N)
  y <- sin(abs(q))/sqrt(abs(q)) #500 + 0.7 * (q-15)^3 + 100 * (q-15)
  noise <- rnorm(length(q), mean=0, sd = sd.noise)
  noisy.y <- y + noise

  #plot(X, Y)
  return(data.frame(X=q+15, Y=noisy.y, Y_true=y))
}

generate.data.poly_old <- function(N, seed, sd.noise = 300) {
  # set.seed(seed = seed)
  # X = rnorm(N)
  # set.seed(seed = seed+1)
  # Y = exp(X-offset) + rnorm(n = length(X), sd = sd.noise)
